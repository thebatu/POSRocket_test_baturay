...
//{key:function()}
function validate(validators, toValidate){

    for(var key in validators){
        if(Array.isArray(toValidate[key])){
            for(var subKey in toValidate[key]){
              console.log(subKey)
                if(!validate(validators[key],toValidate[key][subKey])){
                    return false
                }
            }
        }else if(typeof toValidate[key] === 'object'){
                if(!validate(validators[key],toValidate[key])){
                    return false
                }
        }else if(!(validators[key](toValidate[key]))){
                return false
        }
    }
    return true
